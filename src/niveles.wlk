...
object nivel1 inherits Niveles {
	...
	if (system.turn() == 77) {
		12.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
	}
	if (system.turn() == 81) {
		7.times( { n => waveToSpawn.add(self.inutilDelHacha()) } )
		15.times( { n => waveToSpawn.add(self.inutilDeLaEspada()) } )
		17.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
	}
	if (system.turn() == 85) {
		20.times( { n => waveToSpawn.add(self.inutilNoMuertoConEstilo()) } )
		6.times( { n => waveToSpawn.add(self.inutilQueBufea()) } )
	}
	if (system.turn() == 86) {
		19.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
		13.times( { n => waveToSpawn.add(self.inutilDelHacha()) } )
	}
	if (system.turn() == 90) {
		9.times( { n => waveToSpawn.add(self.inutilDelHacha()) } )
		15.times( { n => waveToSpawn.add(self.inutilDeLaEspada()) } )
	}
	if (system.turn() == 94) {
		8.times( { n => waveToSpawn.add(self.inutilQueBufea()) } )
		13.times( { n => waveToSpawn.add(self.inutilNoMuertoConEstilo()) } )
		14.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
	}
	if (system.turn() == 96) {
		19.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
		14.times( { n => waveToSpawn.add(self.inutilDelHacha()) } )
	}
	if (system.turn() == 97) {
		8.times( { n => waveToSpawn.add(self.inutilDeLaEspada()) } )
		17.times( { n => waveToSpawn.add(self.inutilNoMuertoConEstilo()) } )
	}
	if (system.turn() == 98) {
		1.times( { n => waveToSpawn.add(self.abomination()) } )
		7.times( { n => waveToSpawn.add(self.inutilDelHacha()) } )
		10.times( { n => waveToSpawn.add(self.inutilDeLaEspada()) } )
	}
	if (system.turn() == 100) {
		18.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
		15.times( { n => waveToSpawn.add(self.inutilNoMuertoConEstilo()) } )
	}
	if (system.turn() == 103) {
		15.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
		9.times( { n => waveToSpawn.add(self.inutilQueBufea()) } )
	}
	if (system.turn() == 105) {
		13.times( { n => waveToSpawn.add(self.inutilDeLaEspada()) } )
		1.times( { n => waveToSpawn.add(self.weWillTakeJerusalem()) } )
	}
	if (system.turn() == 106) {
		9.times( { n => waveToSpawn.add(self.inutilNoMuertoConEstilo()) } )
		14.times( { n => waveToSpawn.add(self.inutilNoMuerto()) } )
	}
